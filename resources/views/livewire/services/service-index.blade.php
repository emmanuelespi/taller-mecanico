    ...
    <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
            <label class="flex items-center gap-2 text-sm text-gray-400 cursor-pointer">
                <input type="checkbox" wire:model="showOnlyActive"
            ...
            <button wire:click="$dispatch('openModal', { serviceId: null })"
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="transition-colors border-b border-gray-800 hover:bg-gray-800/50">
                    <td class="px-5 py-3.5 text-xs text-gray-500">{{ $service->id }}</td>
                    <td class="px-5 py-3.5 text-sm font-medium text-white">
                        {{ $service->name }}
                    </td>
                    <td class="px-5 py-3.5 text-sm text-gray-400">
                        {{ Str::limit($service->description, 60) }}
                        @if(strlen($service->description) > 60)
                        <span class="text-xs text-gray-500 cursor-help" title="{{ $service->description }}"> ...ver
                            más</span>
                        @endif
                    </td>
                    ...
                    <td class="px-5 py-3.5">
                        <div class="flex items-center gap-2">
                            <!-- Editar -->
                            <button wire:click="$dispatch('openModal', { serviceId: {{ $service->id }} })"
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div class="w-full max-w-md bg-gray-900 border border-gray-800 shadow-2xl rounded-2xl">
            <div class="flex items-center gap-4 px-6 py-5">
                ...
                <div>
                    <h2 class="text-xl font-bold text-white">¿Eliminar servicio?</h2>
                    <p class="text-sm text-gray-400 mt-0.5">Esta acción eliminará el servicio permanentemente. ¿Estás
                        seguro?</p>
                </div>
            </div>
