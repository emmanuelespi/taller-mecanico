                ...
                <tr class="transition-colors border-b border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td class="px-5 py-3.5">
                        <span
                            ...
                            {{ $user->isAdmin() ? 'bg-purple-500/10 text-purple-400' :
                               ($user->isMechanic() ? 'bg-blue-500/10 text-blue-400' : 'bg-green-500/10 text-green-400') }}">
                            <span class="w-1.5 h-1.5 rounded-full
                                {{ $user->isAdmin() ? 'bg-purple-400' :
                                   ($user->isMechanic() ? 'bg-blue-400' : 'bg-green-400') }}"></span>
                            {{ $user->getRoleName() }}
                        </span>
                    </td>
                    <td class="px-5 py-3.5">
                        <span
                            class="inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded-full
                            {{ !$user->trashed() ? 'bg-green-500/10 text-green-400' : 'bg-gray-500/10 text-gray-400' }}">
                            <span
                                class="w-1.5 h-1.5 rounded-full {{ !$user->trashed() ? 'bg-green-400' : 'bg-gray-400' }}"></span>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button wire:click="toggleActive({{ $user->id }})"
                                class="flex items-center justify-center w-8 h-8 transition-colors rounded-lg
                                    {{ !$user->trashed() ? 'bg-yellow-500/10 text-yellow-400 hover:bg-yellow-500/20' : 'bg-green-500/10 text-green-400 hover:bg-green-500/20' }}"
                                title="{{ !$user->trashed() ? 'Desactivar usuario' : 'Activar usuario' }}">
                                <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    stroke-width="2">
                                    @if(!$user->trashed())
                                    <path
        ...
        <div class="w-full max-w-md bg-gray-900 border border-gray-800 shadow-2xl rounded-2xl">
            <div class="flex items-center gap-4 px-6 py-5">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-400 mt-0.5">Esta acción eliminará el usuario permanentemente. ¿Estás
                        seguro?</p>
                </div>
            </div>
